...
class FindUserInfoTest: AbstractTest() {
    ...
    private fun performOkRequest(request: FindUserInfoRequest): FindUserInfoResponse {
        val mvcResult = mockMvc.perform(getRequest(request))
            .andExpect(status().isOk)
            .andReturn()
        val contentResult = mvcResult.response.contentAsString

        return jsonConverter.fromJson(contentResult, FindUserInfoResponse::class.java)
    }

    private fun getRequest(request: FindUserInfoRequest, containsAuthHeader: Boolean = true): MockHttpServletRequestBuilder {
        val builder = post("/find/user/info")
